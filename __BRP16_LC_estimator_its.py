...
def RPlot(X,Y,method,df,mpe):
    ...
    ax.scatter(X[wkday], Y[wkday], label='Weekdays', c='b',marker='o',ms='10')
    # scatter warning points in red (c='r')
    ax.scatter(X[wkendSat], Y[wkendSat], label='Weekends', c='r',marker='s',ms='10')
    if holi.size > 0:
...
date_index = pd.date_range('1/1/2016', periods=366, freq='D')
#AU_holidays = holidays.Australia()
...
mypath17 = r"LC data\2016 Link data"
numLinks = len(indf)
i = 0
dataframes = ''
            ...
            if len(dfXW.columns) == 4 and len(dfYW.columns) == 4:
                ...
                if (len(dfXW.columns) == 4 and len(dfYW.columns) == 4):
                    ...
                    if (len(dfXW.index) > 2 and len(dfYW.index) > 2 and len(dfXW) > 20 and len(dfXW) > 20):
                        ...
                        holi = np.asarray(index_holidays)
                        methodx = "Estimation at link id " + str(O) + " at: " + str(f[14:16]) + ':' + str(f[16:18])
                        #if O == '2903-2902' and f =='LinkTrainData_0800_2017.csv': #  LinkTrainData_0800_2017.csv
                        #if (mpe < 10 and str(O) == '2924-2925'):
...
if 1 == 1:
    ...
    erdfBir.to_csv('rmseBi16y.csv')
    erdfBi.to_csv('erdfBi16y.csv')
    crdf.to_csv('crdf16y.csv')
    rawdf.to_csv('rawdata16y.csv')
##    #erdf.dropna(axis=1)
    dcidf.to_csv('noData16y.csv')
##    
...
real_pred_df.explode(['RealX','PredX']).to_csv('real_pred_df16_brp.csv')
