...
date_index = pd.date_range('1/1/2019', periods=365, freq='D')
#AU_holidays = holidays.Australia()
...
mypath17 = r"LC data\2019 Link data"
numLinks = len(indf)
i = 0
dataframes = ''
...
for inxf, f in enumerate(filenames17[:-1]):
    ...
    for dow in np.arange(0,7):
    ####### this loop is day of week ###     
        ## only select same days
        dfTraindataX = TraindataX[TraindataX.index.dayofweek == dow]
        dfTraindataY = TraindataY[TraindataY.index.dayofweek == dow]
        ## only select same days
        numberOfData = []
        rmseX = []
        rmseY = []
                ...
                if (len(dfXW.columns) == 4 and len(dfYW.columns) == 4):
                    ...
                    if (len(dfXW.index) > 2 and len(dfYW.index) > 2 and len(dfXW) > 20 and len(dfXW) > 20):
                        ...
                        if ((mpe < 100) and (rmse < 100)):
                            ...
                            nodatadf.iloc[nodatadf.index.tolist().index(xi)][ColTimes[inxf]] =  nOd/52
...
if 1 == 1:
    ...
    erdfBir.to_csv('rmseBi19d.csv')
    erdfBi.to_csv('erdfBi19d.csv')
##    crdf.to_csv('crdf18d.csv')
    rawdf.to_csv('rawdata19d.csv')
##    #erdf.dropna(axis=1)
    dcidf.to_csv('noData19d.csv')
##    
...
##    rvp.to_csv('RvP18d.csv')
real_pred_df.explode(['RealX','PredX']).to_csv('real_pred_df19_mrp.csv')
